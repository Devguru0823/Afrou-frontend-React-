import {
  SET_TALENT_FEED,
  UPDATE_TALENT_FEED_COMMENT,
  UPDATE_TALENT_FEED_COMMENT_LIKE,
  UPDATE_TALENT_FEED,
  UPDATE_TALENT_FEED_TEXT,
  UPDATE_TALENT_FEED_DELETE,
  CLEAR_TALENT_FEED,
  UPDATE_TALENT_FEED_COMMENT_REPLY,
  UPDATE_TALENT_COMMENT_MANIPULATE,
  UPDATE_TALENT_FOLLOW,
} from './actionTypes';
...
export const fetchTalentFeed = (token, pageNumber) => {
  const config = {
    headers: {'Authorization': "bearer " + token}
  };
  return dispatch => {
    dispatch(uiStartLoading());
    axios.get(`/posts/afrotalent?page=${pageNumber}`, config)
      .then(response => {
...
export const postTalentComment = (token, comment, user, feeds) => {
  const config = {
    headers: {'Authorization': "bearer " + token}
  };
  return dispatch => {
    ...
    axios.post('/comments', comment, config)
      .then(response => {
        ...
        if (newCommentData.comment_parent_id === 0 ) {
          dispatch(updateTalentFeedComment(feeds, newCommentData.post_id, newCommentData));
        } else {
          dispatch(updateTalentFeedCommentReply(feeds, newCommentData.post_id, newCommentData.comment_parent_id, newCommentData));
        }
        dispatch(uiStopLoading());
      })
      .catch(err => {
...
export const updateTalentFeed = (feeds, id) => {
  return dispatch => {
...
export const updateTalentFeedComment = (feeds, id, comment) => {
  return dispatch => {
    const newFeeds = [...feeds];
    const feedIndex = newFeeds.findIndex(feed => feed.post_id === id);
...
export const updateTalentFeedPostText = (token, feeds, id, text) => {
  return dispatch => {
    const config = {
      headers: {
...
export const updateTalentFeedDelete = (token, feeds, id) => {
  return dispatch => {
    const config = {
      headers: {
        'Authorization': "bearer " + token
...
export const commentManipulateTalent = (token, feeds, postId, commentId, comments, isEdit, text) => {
  return dispatch => {
    const config = {
      headers: {
        'Authorization': "bearer " + token
    ...
    if (isEdit) {
      axios.put(`/comments/${commentId}`, text, config)
        .then(response => {
          ...
          dispatch(updateTalentCommentDelete(feedIndex, comments, 'edit', count));
        })
        .catch(err => {
          console.log(err);
        });
      ...
      axios.delete(`/comments/${commentId}`, config)
        .then(response => {
          ...
          const feedIndex = newFeeds.findIndex(feed => feed.post_id === postId);
          const count = response.data.data ? response.data.data.comment_count : 0;
          dispatch(updateTalentCommentDelete(feedIndex, comments, 'delete', count));
        })
        .catch(err => {
          console.log(err);
        });
...
const updateFeed = index => {
  return {
    type: UPDATE_TALENT_FEED,
    feedIndex: index,
  }
};

export const updateTalentFeedCommentLike = (feeds, id, commentId) => {
  return dispatch => {
    const newFeeds = [...feeds];
    const feedIndex = newFeeds.findIndex(feed => feed.post_id === id);
...
const updateFeedComment = (index, comment) => {
  return {
    type: UPDATE_TALENT_FEED_COMMENT,
    feedIndex: index,
    comment: comment
  }
};

export const updateTalentFeedCommentReply = (feeds, id, commentId, comment) => {
  return dispatch => {
    const newFeeds = [...feeds];
    const feedIndex = newFeeds.findIndex(feed => feed.post_id === id);
...
const updateFeedCommentLike = (index, commentIndex, commentChildIndex) => {
  return {
    type: UPDATE_TALENT_FEED_COMMENT_LIKE,
    feedIndex: index,
...
const updateFeedCommentReply = (index, commentIndex, comment) => {
  return {
    type: UPDATE_TALENT_FEED_COMMENT_REPLY,
    feedIndex: index,
    commentIndex: commentIndex,
    comment: comment
  }
...
const updateFeedText = (index, text) => {
  return {
    type: UPDATE_TALENT_FEED_TEXT,
    feedIndex: index,
    text: text
  }
};

export const resetTalentFeed = () => {
  return {
    type: CLEAR_TALENT_FEED
  }
};

const updateFeedDelete = index => {
  return {
    type: UPDATE_TALENT_FEED_DELETE,
    feedIndex: index
  }
};

const updateTalentCommentDelete = (index, comments, type, count) => {
  return {
    type: UPDATE_TALENT_COMMENT_MANIPULATE,
    feedIndex: index,
    comments: comments,
    manipulateType: type,
    count: count
...
export const updateTalentFollow = id => {
  return {
    type: UPDATE_TALENT_FOLLOW,
    payload: id
  }
};
