...
class EditItem extends Component {
  ...
  handleSubmit = (event) => {
    ...
    for (let formElementIdentifier in this.state.orderForm) {
      if (this.state.orderForm[formElementIdentifier].value) {
    ...
    axios.put(`/market/${this.props.data.market_post_id}`, fData, config)
      .then(response => {
  ...
  render() {
      ...
      <form onSubmit={this.handleSubmit}>
        <div className="row">
          ...
          <div className="form-group col-md-3">
            {data ? <img src={`${BASE_URL}${data.main_image}${SIZE_45}`} className="market-show-editpageiamge"/> : null}
            <span>Main Image</span>
          </div>
          {
            data && data.sub_image1 ?
              <div className="form-group col-md-3">
                <img src={`${BASE_URL}${data.sub_image1}${SIZE_45}`} className="market-show-editpageiamge"/>
                <span>Gallery 1</span>
              </div> : null
          }
          {
            data && data.sub_image2 ?
              <div className="form-group col-md-3">
                <img src={`${BASE_URL}${data.sub_image2}${SIZE_45}`} className="market-show-editpageiamge"/>
                <span>Gallery 2</span>
              </div> : null
          }
          {
            data && data.sub_image3 ?
              <div className="form-group col-md-3">
                <img src={`${BASE_URL}${data.sub_image3}${SIZE_45}`} className="market-show-editpageiamge"/>
                <span>Gallery 3</span>
              </div> : null
          }
          <div className="form-group col-md-12">
            <button type="submit" disabled={!this.state.formIsValid || this.state.isLoading} className="btn btn-primary">
              {!this.state.isLoading ? 'Edit market post' : 'Loading...'}
            </button>
          </div>
        </div>
      </form>
    ...
    return (
      <div id="createmarket">
        ...
        <div className="topboxtxt">Edit Market</div>
